...
Із `inputTransposing : 1` ширина кроку буде `[ 2, 1 ]`, а не `[ 1, 2 ]` як в попередньому прикладі. Послідовність виглядає так:
...
Приведена діаграма показує як буфер відображається ( map into ) в матрицю. Всі скаляри йдуть послідовно один за одним. За замовчуванням `strides` обраховується так щоб всі скаляри йшли послідовно. В якій же послідовності йдуть рядок та колонка вказує опція `inputTransposing`.
