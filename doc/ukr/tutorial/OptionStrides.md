...
Об'єкт класу `Matrix` можливо створити одним із варіантів задання значень полів об'єкта.
...
Якщо задати значення опції `inputTransposing : 1` тоді ширина кроку даного прикладу буде `[ 2, 1 ]`:
...
Приведена діаграма показує як буфер відображається ( map into ) в матрицю. Всі скаляри йдуть послідовно один за одним. За замовчуванням `strides` обраховується так щоб всі скаляри йшли послідовно. В якій же послідовності перебирати розмірності вказує опція `inputTransposing`.
...
Транспонування матриці можливо здійснити без пересування даних в буфері матриці. Транспонування квадратної матриці можливо виконати лише зміною ширини кроку( strides ). В наступному прикладі здійснюється zero-copy транспонування матриці.
