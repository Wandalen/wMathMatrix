...
Статичною рутина `Mul` можливо перемножити й декілька матриць за раз.
...
Послідовність запису матриць в масиві віповідає порядку множення. Матриці послідовно перемножуються одна на одну. Так як перший аргумент має значення `null` то під результат свторюється новий контейнер `dst`, який і повертається, як результат виконання рутини. Жодна із матриць `matrix1`, `matrix2`, `matrix3` не змінюється.
